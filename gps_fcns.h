...
#define GPS_INT_PIN 2


SoftwareSerial gpsSerial(GPS_RX_PIN, GPS_TX_PIN); //gpsSerial.begin(9600);

SEMAPHORE_DECL(gps_sem, true);

void gps_init()
{
  //Attaching timer1 interrupt right after first falling edge of the GPS
...
void gps_log()
{
  int gps_bytes_len = gpsSerial.available();
  if(gps_bytes_len > 0)
  {
    char * gps_in_buffer;
    int16_t gps_in_buffer_idx = 0;
    gps_in_buffer = new char[gps_bytes_len];
    memset(gps_in_buffer, 0, sizeof(char)*gps_bytes_len);
    gps_in_buffer_idx = gpsSerial.readBytesUntil((char)13, gps_in_buffer, gps_bytes_len);
    if(gps_in_buffer_idx > 72)
    {
      Serial.println(gps_in_buffer_idx);
    }else{
      delete [] gps_in_buffer;
    }
  }
  gps_ready = false;
} */
...
void gps_log()
{
  ...
  while(millis()-start < 10)
  {
    ...
    {
      Serial.println((char*)gpsSerial._receive_buffer);
    }
  }
}

NIL_WORKING_AREA(waThread2, 144);
...
void setup()
{
  ...
  //ON  = 0x01
  //
  byte settingsArray[] = {0x03, //NavMode
                          0xFA, 0x00, //DataRate
                          0x00, 0xE1, 0x00, //PortRate
                          0x00, 0x00, 0x00, 0x00, 0x00}; //NMEA OFF/ON
...
void configureUblox(byte *settingsArrayPointer) {
  ...
  // Where the config is actually written to the GPS module
  gpsStatus = new boolean[7];
  memset(gpsStatus, false, sizeof(boolean)*7);

  while(gpsSetSuccess < 3 &(!gps_enable) )
  {
